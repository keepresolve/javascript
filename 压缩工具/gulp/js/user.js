...
$(function(){
			...
			if(d.code==200){
				...
				if(parseInt(eConfig.advertisement)==2&&terminaltype!= "0"){
					...
					image.onload=function(){
						var showHeight = this.height/(this.width/160)+65;
						$(".announcement_con").css("height","calc(100% - "+showHeight+"px)")
					}
	...
	$(".msg_cont").focus(function(){
	    ...
	    if(u.indexOf("iphone")>-1){
			setTimeout(function(){
				...
				$(".direct-chat-messages").scrollTop($(".direct-chat-messages").get(0).scrollHeight-$(".direct-chat-messages").height())
			},500)
	    }
	...
	function $msgStr(code,type,direction,message,time,name){
		...
		$(".direct-chat-messages").scrollTop($(".direct-chat-messages").get(0).scrollHeight-$(".direct-chat-messages").get(0).offsetHeight)
	}
	//获取历史记录
