$(function(){
	...
	var canLogin;//控制socket连上后的处理
	var closeSocketTimer;
	...
	debug("created a socket:" + socket.id)

	debug('没有强制用websocket')
	function cancelSocketTimer () {
		debug('点击转人工或者重连客服的时候要查是不是起了这个timer:'+closeSocketTimer)
		if (closeSocketTimer) {
			debug('取消timer')
			clearTimeout(closeSocketTimer);
			closeSocketTimer = false
		}
	}

	function closeSocket() {
		 var sid = socket.id;
		 debug('要关闭的socket id:'+sid)
		 closeSocketTimer = setTimeout(function(){
			if (socket.connected) {
				debug('三分钟后还没关闭，认为用户也想不点评价了，我们关闭socket')
				if (socket.id == sid) {
					debug('确实还是这个socket，关闭它')
					socket.disconnect()
					closeSocketTimer = false;
				} else {
					debug('好像我们取消timer都考虑到后，这种情况不存了：socket id已变:'+socket.id)
					closeSocketTimer = false;
				}
			} else {
				debug('看来用户评价了，所以已经关闭socket')
			}
		},180*1000);
	}

	function staffService (completeList) {
		cancelSocketTimer();
		if (socket.connected) {
			debug('socket connected so swith to staff service')
			socket.emit("switchPosition", {
				roomID: getroomID()
			},function(data){
				debug(data)
				// 自动补全消失
				if (completeList) $(".completeList") && $(".completeList").hide()
				kefuAccess(data,"notgethistory")
			});
		} else {
			debug('socket NOT connected so connect first before swith to staff service')
			canLogin = (completeList)? 3 :4
			socket.connect();
		}
	}

	socket.on('connect',function(data){
		debug("socket连接状态:" + socket.id + ": "+ socket.connected)
		//简单处理直接调用issession(1)不做判断
		issession(1)
		switch(canLogin) {
			case 2: 
				debug('重连socket')
	...
	socket.on('close_session',function(data){
		...
		if (data.code ==1 ) {
			debug('机器人超时，不用评价，所以关闭socket')
			socket.disconnect()
		} else {
			debug('还需要用户发评价，所以此时不能关闭socket')
			closeSocket();
		}
	})
	//接收typing事件
	...
	socket.on('reconnect',function(){
		//https://github.com/socketio/socket.io/issues/3269
		// issession(1)
		debug("reconnect:" + socket.id)
		//重连成功后直接重刷页面，丢弃原来的socket不好，要改下
		location.replace(location.href);
	})

	//socket重新连接失败,这步应该不需要因为disconnect事件已经处理
	socket.on('reconnect_failed', function() {
		issession(0)
		debug("reconnect_failed:"+socket.id);
	});
	socket.on('reconnect_attempt', function(data){
		debug("reconnect_attempt:" + socket.id)
	});
	socket.on('reconnecting', function(data){
		debug("reconnecting:" + socket.id)
	});


	//转人工
	$(".switch_position").click(function(){	
		staffService('completeList')
	})
	...
	$("body").on("click",".evaluation_btn",function(){
		...
		var cluid = $(this).parents(".direct_chat_msg").attr("cluid")
	...
	$("body").on("click",".score_btn",function(){
		...
		socket.emit("score",{roomID:roomID,score:val},function(data){
			debug('可能是对以前的会话做评价，所以不要关闭socket,交由closeSocketTimer处理:'+closeSocketTimer)
		})
	...
	$(".reconnect_kefu,.wap_reconnect_kefu").click(function(){
		...
		// 	return;
		// }
		debug('点击重新连接客服，原来不关闭socket的方法不妥，重新连接socket')
		canLogin=2
		socket.connect();
		if (!socket.connected) {
			debug('目前disconnected,等待连接结果再做后续处理')
			return;
		}
		debug('目前就处在connected!')
		cancelSocketTimer();
		issession(1);
		userlogin("notgethistory")
	})
	...
	function kefuAccess(data,notgethistory){
		...
		if (data.access == 10 || data.access == 11 || data.access == 1 || data.access == 2){
			isswitchPosition(1)
		}else{
			isswitchPosition(0)
		}
			...
			case "operation"://系统消息及操作消息
				switch(direction){
					...
					case "robotscore"://机器人答案评价
						...
						var $msg = $('<div style="min-width:170px;" class="direct-chat-msg  direct_chat_msg left msg_left" skipTime="' + time + '" aid="' + message.aId + '" cluid="' + message.cluid + '">\
									<div class="direct-chat-info pull-left direct-chat-info-name">'+name+'</div>\
									<div class="direct-chat-text">\
