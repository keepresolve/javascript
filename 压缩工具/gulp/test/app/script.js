(function () {
	...
	var debug = require('debug')('user:script')
	 
	script.onload = function () {
		var $ = jQuery.noConflict(true);	
		debug({insideVersion:"内部jquery版本"+$.fn.jquery})
		//获取企业配置信息
		var eConfig = {};
		//是否需要刷新iframe
		...
		//var url=""//本地
		// target = $("#"+target).length>0?target:undefined//判断当前元素是否存在
		//获取配置创建入口图标
		$.ajax({
			type: "GET",
			...
			success: function (d) {
				...
				if (d.code == 200) {
					...
					var icon = $('<div id="YSF-CUSTOM-ENTRY-0" style="z-index:9999; position: fixed;right: 30px;bottom: 30px;width: 90px;height: 40px;background-color: ' + d.data.mainColor + ';border-radius: 20px;box-shadow: 0px 6px 10px 0px rgba(0,0,0,0.25);cursor: pointer;">\
					...
					var icon1 = $('<div id="YSF-CUSTOM-ENTRY-0" style="z-index:9999; position: fixed;right: 30px;bottom: 30px;width: 40px;height: 85px;background-color: ' + d.data.mainColor + ';border-radius: 20px;box-shadow: 0px 6px 10px 0px rgba(0,0,0,0.25);cursor: pointer;">\
		...
		window.addEventListener('message', function (e) {
			...
			switch (e.data.key) {
				...
				case "miHuaImgSrc":
					...
					debug(src)
					$(".show_box").remove()
					var $img = $('<div class="show_box"  scale="1" style="position: absolute;width:800px;height: 600px;top: calc(50% - 300px);left: calc(50% - 400px);z-index: 10000;background:#ffffff;border:1px solid #eeeeee;box-shadow:0 2px 5px 0 rgba(50,50,50,0.50);border-radius:2px;">\
						<img src="'+ url + '/user/img/Shapeclose.png" class="show_img_close" style="z-index:10000;position: absolute;top: 10px;right: 10px;">\
		...
		function openChat() {
			...
			if ($("#YSF-PANEL-INFO").length == 1) {
				...
				var chat = $('<div id="YSF-PANEL-INFO" class="ysf-chat-layer"  style="max-width:' + chat_width + ';width:' + chat_width + ';height: 600px; z-index:9999; box-shadow: 0 0 20px 0 rgba(0, 0, 0, .15);position: fixed;bottom: -600px;right: 10px;transition: bottom 0.5s;">\
