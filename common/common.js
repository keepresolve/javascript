...
export const gdRequest = (method, url, param, headerObj) => {
    ...
    //debounce 变种;返回 cached promise
    let cached = requestArr[key],
        isDelay = isDebounce(option),
        time = 0,
        status = 1,
        result
    ...
    requestArr[key] = { time: Date.now(), result: result, status }
    console.log({ requestArr, key: requestArr[key], status })
    return result.then(res => {
        requestArr[key]['status'] = 1
