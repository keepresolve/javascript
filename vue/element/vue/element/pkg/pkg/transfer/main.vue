          ...
          <c-scrollbar style="height: 100%">
            <ul>
              <li class="checked-item" v-for="(item,index) in showcheckedList" :key="index">
                <span
                  :class="{'ellipsis':true,'disableColor':item[propDisabled]}"
                  :title="item[propLabel]"
                >{{item[propLabel]}}</span>
  ...
  computed: {
    ...
    rightHeaderString() {
      if (!this.format.right || typeof this.format.right != "string")
        return `已选择 <span style="font-size: 12px;color: #E6A441;">${
          this.checked.length
        }</span> 个`;
      return this.format.right
        .replace(
          /\${checked}/g,
          ` <span style="font-size: 12px;color: #E6A441;">${
            this.checked.length
          }</span>`
        )
