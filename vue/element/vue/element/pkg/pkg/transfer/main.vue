        ...
        <div class="cTransfer-wrapper">
            ...
            <div class="cTransfer-slide" v-loading="loading">
                ...
                <div class="cTransfer-slide-header" :style="{marginBottom:hiddenSelect?'14px':''}">
                    <el-input v-model="query" :placeholder="placeholder" prefix-icon="el-icon-search" class="input-34">
                    </el-input>
                    <!-- 全选 -->
                    <div>
                        <el-checkbox style="padding:10px 0px;" v-if="!hiddenSelect" :indeterminate="isIndeterminate" v-show="filterData.length>0" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
                    </div>
                </div>
                <div class="cTransfer-slide-body" :style="{height:hiddenSelect?'calc(100% - 80px)':'calc(100% - 105px)'}">
                        ...
                        <el-checkbox-group v-model="checked" v-if="!hiddenSort">
                            <div v-for="(item, index) in letterList" :key="index">
                                ...
                                <div v-for="(subitem, subindex) in item.data" :key="subindex">
                                    <el-checkbox :label="subitem.key" :key="subitem.key" :title="subitem.label" :disabled="subitem.disabled">{{subitem.label}}</el-checkbox>
                                </div>
                            </div>
                        </el-checkbox-group>
                        <el-checkbox-group v-model="checked" v-if="hiddenSort">
                            <div v-for="(item, index) in filterData" :key="index">
                                <el-checkbox :label="item.key" :key="item.key" :title="item.label" :disabled="item.disabled">{{item.label}}</el-checkbox>
                            </div>
                        </el-checkbox-group>
                    </el-scrollbar>
                </div>
...
.cTransfer .cTransfer-slide-body .el-scrollbar__wrap {
...
export default {
    ...
    watch: {
        value(n, o) {
            ...
            if (n.find(v => o.indexOf(v) == -1) || n.length != o.length) {
                this.checked = n.map(v => String(v));
            }
        },
        checked(n, o) {
            if (n.length > 0 && this.filterData.length > 0) {
    ...
    computed: {
        ...
        filterData() {
            ...
            this.checkAll = !Boolean(
                list.find(
                    v => this.checked.indexOf(String(v[this.propKey])) == -1
                )
            );
            ...
            return list.map(v => {
                return {
                    ...
                    disabled: Boolean(v[this.propDisabled])
                };
            });
        },
        letterList() {
